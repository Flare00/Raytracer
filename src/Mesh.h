...
public:
    ...
    RayTriangleIntersection intersect( Ray const & ray ) const {
        ...
        //DO iterer sur les triangles et calculer l'intersection la plus proche

        closestIntersection.t = FLT_MAX;
        for(int i = 0, max = triangles.size(); i < max; i++){
            RayTriangleIntersection temp = Triangle(vertices[triangles[i][0]].position, vertices[triangles[i][1]].position, vertices[triangles[i][2]].position).getIntersection(ray);
            if(temp.intersectionExists){
                if(temp.t < closestIntersection.t){
