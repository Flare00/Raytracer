...
public:
    ...
    RaySphereIntersection intersect(const Ray &ray) const {
        ...
        //t²d.d + 2t d.(o-c) + ||o-c||² -r² = 0
        //b² - 4ac
        // a = t²d.d ; b = 2t d.(o-c) ; c = ||o-c||² -r² 
        float a = Vec3::dot(ray.direction(), ray.direction());
        float b = 2*Vec3::dot(ray.direction(), (ray.origin() - m_center));
        float c = (ray.origin() - m_center).squareNorm() - m_radius*m_radius;
        ...
        if(delta >= 0){
            ...
            if(t1 > 0 || t2 > 0){
                ...
                intersection.t = (t1 < 0) ? t2 : ((t2 < 0) ? t1 : std::min(t1,t2));
                intersection.intersection = ray.origin() + (ray.direction()*intersection.t);
                if(t1 > 0 && t2 > 0){
                    intersection.secondintersection = ray.origin() + (ray.direction()*std::max(t1,t2));
                }
                Vec3 sphericalCoord = EuclideanCoordinatesToSpherical(intersection.intersection);
                intersection.theta = sphericalCoord[0];
                intersection.phi = sphericalCoord[1];
